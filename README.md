...
支持 DeepSeek、OpenRouter、Kimi、通义千问、智谱、硅基流动等 OpenAI-compatible `/chat/completions` 接口。网页配置会自动写入 `.env`；点“清除 Key”会同时清空网页输入和 `.env` 里的 `AI_API_KEY`。
